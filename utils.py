...
def count_TP_FP_FN(rmse_vec, thres, label):
    n_TP = 0
    n_FP = 0
    n_FN = 0
    n_TN = 0

    for i in range(len(rmse_vec)):
        if rmse_vec[i] >= thres:
            judge = 1
        else:
            judge = 0
...
def compute_precision_and_recall(rmse_vec, thres, label):
    # compute true positive
    n_TP, n_FP, n_FN, n_TN = count_TP_FP_FN(rmse_vec, thres, label)

    if n_TP + n_FP + n_FN + n_TN == 0:
        accuracy = 1
    else:
    ...
    return accuracy, precision, recall, F1, n_TP, n_FP, n_FN